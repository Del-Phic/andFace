...
- A manual occlusion checkbox is treated as a hint, not as trusted evidence.
...
5. Convert landmarks into 170 normalized observable features. Optional iris landmarks are used when MediaPipe returns 478 points; otherwise iris-derived features are marked observable=false and excluded from scoring.
...
```text
finalScore = 0.70 * fuzzyScore + 0.30 * mahalanobisScore
```
...
The app uses 170 normalized observable features. The first 13 are the original baseline features, and the rest add finer eye, brow, nose, upper/mid-face, cheek, and optional iris-derived geometry from MediaPipe landmarks. The full feature enum lives in `app/src/main/java/dev/andface/galaxy/feature/FeatureType.kt`.
...
- Repeated risky failure lockout.
- Strict failure reasons.

After the two-second confirmation, the visible `인증 완료` state remains latched
through no-face and transient-quality frames, including clean/mask/left-patch/
right-patch transitions for the same user. A different registered success clears
the old completion immediately and must complete its own two-second confirmation;
repeated identity disagreement from an unregistered person also clears the latch.
Multiple faces, excessive occlusion, hint mismatch, and security/storage/model
failures revoke completion immediately. This state is UI-only and is not a
hardware actuator signal.
...
To reproduce the current final debug APK, including all 277 tests, Android lint,
...
Generated APKs, papers, reports, device captures, and submission packages are
kept outside this source repository.

## Current Verification

Last workspace verification:

```text
testDebugUnitTest: 281 tests, 0 failures, 0 errors
lintDebug: passed, 0 errors
...
Enrollment separation: a new USER profile is rejected with LOW_MARGIN when its clean 48-feature baseline, or the visible subset needed for mask/glasses/patch authentication, is too close to an existing different USER profile.

Real Galaxy validation still requires running the APK on device and recording
the checklist results for USER_1, USER_2, USER_3, masks, glasses, eye patches,
...
### S114 Upper-Face Contour Landmark Note
